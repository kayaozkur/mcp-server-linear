...
import { INTELLIGENCE_TOOLS, ProjectHealthAnalysisSchema, IssueIntelligenceSchema, TeamWorkflowOptimizationSchema, WorkflowRecommendationsSchema, ProjectPredictionSchema, PatternSearchSchema, AutomationOpportunitiesSchema, IntegrationOptimizationSchema, KeyboardShortcutTrainingSchema, LinearIntelligenceStatsSchema, ContextualHelpSchema } from './intelligence-tools.js';
import { INTELLIGENCE_STATS, findPatternsByCategory, searchPatterns } from './generated-linear-intelligence.js';
// Standard Linear MCP tools (maintaining compatibility)
const STANDARD_LINEAR_TOOLS = [
    {
...
export class EnhancedLinearMCPServer {
    ...
    async handleAnalyzeProjectHealth(args) {
    ...
    async handleAnalyzeIssueIntelligence(args) {
    ...
    async handleOptimizeTeamWorkflow(args) {
    ...
    async handleGenerateWorkflowRecommendations(args) {
    ...
    async handleSearchIntelligencePatterns(args) {
        const { query, category, maxResults, includeContext } = PatternSearchSchema.parse(args);
        let patterns;
        if (category) {
            patterns = findPatternsByCategory(category);
            patterns = patterns.filter(p => p.pattern.toLowerCase().includes(query.toLowerCase()) ||
    ...
    async handleIdentifyAutomationOpportunities(args) {
    ...
    async handleOptimizeIntegrations(args) {
        ...
        const recommendations = filteredPatterns.slice(0, 8).map(pattern => ({
            ...
            use_cases: pattern.use_cases,
            already_configured: currentIntegrations?.some(integration => pattern.pattern.toLowerCase().includes(integration.toLowerCase())) || false
        }));
        return {
            content: [
                {
    ...
    async handleGenerateShortcutTraining(args) {
        ...
        if (focusArea) {
            relevantShortcuts = relevantShortcuts.filter(p => p.description.toLowerCase().includes(focusArea.toLowerCase()));
        }
    ...
    async handleGetIntelligenceStats(args) {
        ...
        if (includeBreakdown) {
            stats.category_breakdown = INTELLIGENCE_STATS.categoryCounts;
            stats.complexity_distribution = INTELLIGENCE_STATS.complexityCounts;
            stats.safety_distribution = INTELLIGENCE_STATS.safetyLevels;
        }
    ...
    async handleGetContextualHelp(args) {
    ...
    async handleGetIssueStates(args) {
        const { teamId } = args;
    ...
    async handleGetTeamLabels(args) {
        const { teamId } = args;
