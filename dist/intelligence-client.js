 ...
 */
import { LINEAR_INTELLIGENCE_PATTERNS, INTELLIGENCE_CATEGORIES, findPatternsByCategory, searchPatterns } from './generated-linear-intelligence';
export class LinearIntelligenceClient {
     ...
     */
    analyzeProjectHealth(project, issues) {
        const totalIssues = issues.length;
        const completedIssues = issues.filter(issue => issue.state.type === 'completed' || issue.state.name.toLowerCase().includes('done')).length;
        const inProgressIssues = issues.filter(issue => issue.state.type === 'started' || issue.state.name.toLowerCase().includes('progress')).length;
     ...
     */
    optimizeTeamWorkflow(team, issues) {
        const totalIssues = issues.length;
     ...
     */
    predictProjectOutcome(project, issues, historicalData) {
        const totalIssues = issues.length;
        const completedIssues = issues.filter(issue => issue.state.type === 'completed' || issue.state.name.toLowerCase().includes('done')).length;
        ...
        else if (project.health === 'atRisk')
            completionProbability -= 15;
        else if (project.health === 'offTrack')
     ...
     */
    getContextualHelp(operation, _context = {}) {
    ...
    identifyRiskIndicators(factors) {
        const risks = [];
    ...
    calculatePriorityRecommendation(issue, complexityScore) {
    ...
    generateWorkflowSuggestions(issue) {
        const suggestions = [];
        if (!issue.assignee) {
    ...
    findRelatedPatterns(text) {
        return searchPatterns(text)
            .slice(0, 3)
            .map(pattern => pattern.pattern);
    }
    identifyAutomationOpportunities(issue) {
        const opportunities = [];
    ...
    generateOptimizationSuggestions(efficiencyScore, bottlenecks) {
        const suggestions = [];
    ...
    generateCollaborationImprovements(_team, issues) {
        const improvements = [];
    ...
    identifyProjectRiskFactors(project, issues) {
        const risks = [];
    ...
    identifySuccessIndicators(project, issues, completionRate) {
        const indicators = [];
